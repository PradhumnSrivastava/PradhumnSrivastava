<!-- ========================================================= -->
<!--                       HERO SECTION                         -->
<!-- ========================================================= -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:0D1117,50:1F6FEB,100:38BDF8&text=Pradhumn%20Kumar%20Srivastava&fontColor=FFFFFF&fontSize=48&fontAlignY=38&desc=AI%20Engineer%20•%20Machine%20Learning%20Engineer%20•%20Data%20Scientist&descAlignY=58"/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=24&duration=3500&pause=1000&color=58A6FF&center=true&vCenter=true&width=850&lines=Building+Production+AI+Systems;Machine+Learning+%7C+Deep+Learning;LLMs+%7C+RAG+%7C+Agentic+AI;MLOps+%7C+AI+Engineering;Open+Source+%7C+Research+%7C+Innovation"/>

<br><br>

<a href="https://github.com/PradhumnSrivastava">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/pradhumn-srivastava">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:srivastavapradhumn@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<!-- Uncomment when portfolio is ready
<a href="https://your-portfolio.com">
  <img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=googlechrome&logoColor=white"/>
</a>
-->

<br><br>

<img src="https://komarev.com/ghpvc/?username=PradhumnSrivastava&label=Profile%20Views&style=for-the-badge&color=58A6FF"/>

<img src="https://img.shields.io/github/followers/PradhumnSrivastava?style=for-the-badge&label=Followers&color=58A6FF"/>

<img src="https://img.shields.io/github/stars/PradhumnSrivastava?affiliations=OWNER&style=for-the-badge&label=Total%20Stars&color=58A6FF"/>

</div>

---

<div align="center">

### Building intelligent AI systems that solve real-world problems through Machine Learning, Deep Learning, LLMs, RAG, Agentic AI, and MLOps.

</div>### Currently Building

- Medical Insurance Cost Prediction (MLOps)
- Universal OCR AI
- RAG Chatbot
- Agentic AI Workflows
- Production AI Pipelines

</td>

<td width="50%">

### Current Learning

- Advanced Machine Learning
- Deep Learning
- LangGraph
- Agentic AI
- LLMOps
- Azure AI

</td>

</tr>

<tr>

<td>

### Research Interests

- Large Language Models
- Retrieval Augmented Generation
- AI Agents
- Vision Language Models
- AI Infrastructure
- Model Optimization

</td>

<td>

### Career Goal

Building scalable AI products that solve real-world problems through Machine Learning, LLMs, MLOps and Agentic AI.

</td>

</tr>

</table>

---

# AI Dashboard

<table>

<tr>

<th>Current Focus</th>
<th>Current Project</th>

</tr>

<tr>

<td>

Production AI Engineering

</td>

<td>

Medical Insurance Cost Prediction

</td>

</tr>

<tr>

<th>Learning</th>

<th>Research</th>

</tr>

<tr>

<td>

Agentic AI + LangGraph

</td>

<td>

Autonomous AI Systems

</td>

</tr>

<tr>

<th>Current Tech</th>

<th>Target Role</th>

</tr>

<tr>

<td>

Python • ML • DL • LLMs • RAG • FastAPI

</td>

<td>

AI Engineer

</td>

</tr>

</table>

---

# Engineering Focus

```text
Artificial Intelligence
        │
        ├── Machine Learning
        │
        ├── Deep Learning
        │
        ├── Computer Vision
        │
        ├── NLP
        │
        ├── Generative AI
        │
        ├── RAG
        │
        ├── Agentic AI
        │
        ├── MLOps
        │
        └── Production Deployment
```

---

# Tech Stack

## Programming Languages

<p>

<img src="https://skillicons.dev/icons?i=python,cpp,c,java"/>

</p>

---

## Data Analysis & Visualization

<p>

<img src="https://skillicons.dev/icons?i=anaconda"/>

<img src="https://img.shields.io/badge/Pandas-0D1117?style=flat&logo=pandas"/>

<img src="https://img.shields.io/badge/Numpy-0D1117?style=flat&logo=numpy"/>

<img src="https://img.shields.io/badge/Matplotlib-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Seaborn-0D1117?style=flat"/>

</p>

---

## Machine Learning

<p>

<img src="https://skillicons.dev/icons?i=scikitlearn"/>

<img src="https://img.shields.io/badge/XGBoost-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/LightGBM-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/CatBoost-0D1117?style=flat"/>

</p>

---

## Deep Learning

<p>

<img src="https://skillicons.dev/icons?i=tensorflow,pytorch"/>

<img src="https://img.shields.io/badge/Keras-0D1117?style=flat&logo=keras"/>

</p>

---

## Natural Language Processing

<p>

<img src="https://img.shields.io/badge/NLTK-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/spaCy-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Transformers-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/SentenceTransformers-0D1117?style=flat"/>

</p>

---

## Generative AI

<p>

<img src="https://skillicons.dev/icons?i=openai"/>

<img src="https://img.shields.io/badge/HuggingFace-0D1117?style=flat&logo=huggingface"/>

<img src="https://img.shields.io/badge/LangChain-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/LlamaIndex-0D1117?style=flat"/>

</p>

---

## Agentic AI

<p>

<img src="https://img.shields.io/badge/LangGraph-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/CrewAI-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/AutoGen-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/PydanticAI-0D1117?style=flat"/>

</p>

---

## AI Engineering

<p>

<img src="https://img.shields.io/badge/RAG-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Prompt%20Engineering-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Vector%20Databases-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Embeddings-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/Fine%20Tuning-0D1117?style=flat"/>

</p>

---

## MLOps

<p>

<img src="https://skillicons.dev/icons?i=docker"/>

<img src="https://img.shields.io/badge/MLflow-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/DVC-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/GitHub%20Actions-0D1117?style=flat&logo=githubactions"/>

<img src="https://img.shields.io/badge/FastAPI-0D1117?style=flat&logo=fastapi"/>

</p>

---

## Databases

<p>

<img src="https://skillicons.dev/icons?i=mysql,postgres,mongodb"/>

<img src="https://img.shields.io/badge/FAISS-0D1117?style=flat"/>

<img src="https://img.shields.io/badge/ChromaDB-0D1117?style=flat"/>

</p>

---

## Cloud

<p>

<img src="https://skillicons.dev/icons?i=azure,aws,gcp"/>

</p>

---

## Development Tools

<p>

<img src="https://skillicons.dev/icons?i=git,github,vscode,pycharm,linux,bash"/>

</p>

---

# Philosophy

> Build AI that is **useful**, **reliable**, **scalable**, and **production-ready**.

> Learning never stops. Every project is an opportunity to engineer better systems.

---

<div align="center">

### Turning Ideas into Intelligent Systems

</div>

<!-- ===================== END OF PART 1 ====================== -->
<!-- ========================================================= -->
<!--                    FEATURED PROJECTS                       -->
<!-- ========================================================= -->

# Featured AI Projects

<table>

<tr>

<td width="50%">

## Medical Insurance Cost Prediction

**Production-ready Machine Learning system built with an end-to-end MLOps pipeline.**

**Tech Stack**

Python • Scikit-Learn • MLflow • Docker • DVC • FastAPI • GitHub Actions

**Highlights**

- Complete ML Pipeline
- Data Validation
- Feature Engineering
- Experiment Tracking
- Model Versioning
- CI/CD Integration
- Production Deployment

**Status**

Production Ready

**Complexity**

Advanced

**Repository**

👉 https://github.com/YOUR_USERNAME/medical-insurance-cost-prediction

</td>

<td width="50%">

## Universal OCR AI

OCR platform capable of understanding multilingual documents with AI-powered translation.

**Tech Stack**

Python • PaddleOCR • EasyOCR • Transformers • OpenCV • FastAPI

**Highlights**

- OCR
- Translation
- Layout Understanding
- PDF Processing
- Image Enhancement
- Structured Output

**Status**

Active Development

**Complexity**

Advanced

**Repository**

👉 https://github.com/YOUR_USERNAME/universal-ocr-ai

</td>

</tr>

<tr>

<td>

## Intelligent RAG Chatbot

Context-aware chatbot using modern Retrieval-Augmented Generation architecture.

**Tech Stack**

Python • LangChain • FAISS • Hugging Face • OpenAI • Streamlit

**Highlights**

- Semantic Search
- Vector Database
- Memory
- Prompt Engineering
- Retrieval Pipeline
- Source Attribution

**Status**

Production Ready

**Complexity**

Advanced

**Repository**

👉 https://github.com/YOUR_USERNAME/rag-chatbot

</td>

<td>

## Agentic AI Workflows

Autonomous AI Agents capable of reasoning, planning and tool usage.

**Tech Stack**

LangGraph • CrewAI • Python • OpenAI • FastAPI

**Highlights**

- Multi-Agent Systems
- State Graph
- Tool Calling
- Planning
- Memory
- Workflow Automation

**Status**

Currently Building

**Complexity**

Expert

**Repository**

👉 https://github.com/YOUR_USERNAME/agentic-ai

</td>

</tr>

<tr>

<td>

## Pneumonia Detection

Deep Learning solution for medical image classification.

**Tech Stack**

TensorFlow • CNN • OpenCV • NumPy

**Highlights**

- CNN
- Medical Imaging
- Binary Classification
- Image Preprocessing
- Evaluation Metrics

**Status**

Completed

**Complexity**

Intermediate

</td>

<td>

## AI Resume Screening

Machine Learning powered resume ranking platform.

**Tech Stack**

Python • NLP • Scikit-Learn • Streamlit

**Highlights**

- Resume Parsing
- NLP
- Candidate Ranking
- Similarity Matching

**Status**

Completed

**Complexity**

Intermediate

</td>

</tr>

</table>

---

# Open Source

<table>

<tr>

<th>Area</th>

<th>Focus</th>

</tr>

<tr>

<td>

Machine Learning

</td>

<td>

Reusable ML Pipelines

</td>

</tr>

<tr>

<td>

Generative AI

</td>

<td>

RAG Applications

</td>

</tr>

<tr>

<td>

Agentic AI

</td>

<td>

Workflow Automation

</td>

</tr>

<tr>

<td>

Developer Tools

</td>

<td>

Open Source Utilities

</td>

</tr>

</table>

---

# Current Engineering Roadmap

```text

Production AI Engineering

        │

        ├──────── Advanced Machine Learning

        │

        ├──────── Deep Learning

        │

        ├──────── Computer Vision

        │

        ├──────── Natural Language Processing

        │

        ├──────── Large Language Models

        │

        ├──────── Retrieval Augmented Generation

        │

        ├──────── AI Agents

        │

        ├──────── Agentic AI

        │

        ├──────── MLOps

        │

        ├──────── LLMOps

        │

        ├──────── Cloud AI

        │

        └──────── Production Deployment

```

---

# AI Journey

```text

Commerce

   │

   ▼

Python

   │

   ▼

Data Analysis

   │

   ▼

Machine Learning

   │

   ▼

Deep Learning

   │

   ▼

Computer Vision

   │

   ▼

Natural Language Processing

   │

   ▼

Generative AI

   │

   ▼

Large Language Models

   │

   ▼

Retrieval Augmented Generation

   │

   ▼

Agentic AI

   │

   ▼

MLOps

   │

   ▼

Production AI Engineering

```

---

# Skills Matrix

| Domain | Experience | Current Level | Production Experience |
|---------|------------|---------------|----------------------|
| Python | Strong | Advanced | Yes |
| SQL | Strong | Advanced | Yes |
| Machine Learning | Strong | Advanced | Yes |
| Deep Learning | Growing | Intermediate | Yes |
| Computer Vision | Good | Intermediate | Yes |
| NLP | Strong | Advanced | Yes |
| Generative AI | Strong | Advanced | Yes |
| Prompt Engineering | Strong | Advanced | Yes |
| RAG | Strong | Advanced | Yes |
| LangChain | Strong | Advanced | Yes |
| LangGraph | Learning | Intermediate | In Progress |
| Agentic AI | Learning | Intermediate | In Progress |
| MLOps | Strong | Intermediate | Yes |
| Docker | Learning | Intermediate | In Progress |
| FastAPI | Strong | Intermediate | Yes |
| Azure AI | Learning | Beginner | No |
| AWS | Learning | Beginner | No |

---

# Research Interests

<table>

<tr>

<td>

Large Language Models

</td>

<td>

Reasoning Models

</td>

<td>

AI Agents

</td>

</tr>

<tr>

<td>

Retrieval Systems

</td>

<td>

Vision Language Models

</td>

<td>

Model Optimization

</td>

</tr>

<tr>

<td>

Efficient Fine-Tuning

</td>

<td>

Multimodal AI

</td>

<td>

Production AI

</td>

</tr>

</table>

---

# Current Learning

<table>

<tr>

<th>Technology</th>

<th>Status</th>

</tr>

<tr>

<td>

LangGraph

</td>

<td>

Building Projects

</td>

</tr>

<tr>

<td>

Agentic AI

</td>

<td>

Daily Learning

</td>

</tr>

<tr>

<td>

LLMOps

</td>

<td>

Learning

</td>

</tr>

<tr>

<td>

Azure AI

</td>

<td>

Learning

</td>

</tr>

<tr>

<td>

Production MLOps

</td>

<td>

Implementing

</td>

</tr>

</table>

---

# Certifications

<table>

<tr>

<td>

Microsoft Azure AI

Planned

</td>

<td>

Microsoft Azure Fundamentals

Planned

</td>

<td>

DeepLearning.AI

In Progress

</td>

</tr>

<tr>

<td>

Hugging Face

Planned

</td>

<td>

Google Machine Learning

Planned

</td>

<td>

NVIDIA DLI

Planned

</td>

</tr>

</table>

---

# Development Philosophy

> Production-ready software is built through clean architecture, reproducible experiments, continuous learning, and disciplined engineering—not just trained models.

---

<div align="center">

### Designing AI Systems That Scale Beyond Notebooks

</div>

<!-- ===================== END OF PART 2 ====================== -->
